...
public:
    ...
    bool empty(pair_type const& p)
    {
        return is_end(p.first);
    }

    pair_type empty_queue()
    {
        return pair_type(end(), end());
    }

    pair_type pop_front(pair_type const& p)
    {
        if(empty(p))
            return p;
    ...
    pair_type push_front(pair_type const& p, T const& value)
    {
        list_type new_node = allocate(value, p.first);
        if(empty(p))
            return pair_type(new_node, new_node);
        return pair_type(new_node, p.second);
    }

    pair_type push_back(pair_type const& p, T const& value)
    {
    ...
    struct iterator
    {
        ...
        iterator()
        {
        } // creates a partially formed value
